...
class TaxSite:
    ...
    def __init__(self,taxObj):
        ...
        # 查询扣款异常接口
        self.kk_err_search_url = config['link']['kk_err_search_url']
        # 社保扣款地址
        ...
        # 印花税税源
        self.yhs_base_page_url = config['link']['yhs_base_page_url']
        # 综合申报主界面
        self.other_tax_dic_url = config['link']['other_tax_dic_url']
        ...
        # 获取报税员姓名与密码
        self.tax_staff_info_url = htool.get_link_by_env('tax_staff_ret_url')
        ...
        # 检查发票勾选情况
        self.checked_dkfp_url = htool.get_link_by_env('checked_dkfp_url')
        ...
        # 品目代码配置
        self.pmdm_dic = json.loads(self.template['pmdm_dic'])
    ...
    def open_browser(self):
        ...
        # 根据配置是否显示浏览器
        # if self.show_browser == '0' and self.action != '12':
    ...
    def login(self):
        ...
        self.fuser = {"uuid":""}
        # 获取报税员账号密码
        get_tax_staff = htool.post_data(self.tax_staff_info_url,{"corpid":self.corpid})
        if get_tax_staff.status_code == 200:
            self.staff_data = json.loads(get_tax_staff.text)
            if self.staff_data['code'] != 0:
                print(self.staff_data)
                return False  
        else:
            return False        
        login_ret = self.login_action(5)
        if login_ret == False:
            self.driver.quit()
    ...
    def login_action(self,login_times):
        ...
        try:
            ...
            self.login_verify_submit(driver)

            sleep(4)
            # 判断是否有密码错误的提示框出来 
            layer_dialog_num = driver.execute_script("return $('.layui-layer-dialog').length")
            
            if layer_dialog_num > 0:
                ...
                if '您的密码已输错' in err_msg or '信用代码不能为空' in err_msg or '密码输入错误' in err_msg or '锁定' in err_msg or '请输入密码' in err_msg or '企业户账号不存在' in err_msg:
                    ...
                    # 密码错误
                    htool.post_data(self.tax_password_url,{'corpid':self.corpid,'pwd':'0','tax_staff_pwd':0,'msg':'报税密码错误，请及时修改'})
                    ...
                    driver.quit()
                    return False
                elif '没有绑定已实名认证' in err_msg and '查看' not in err_msg:
                    htool.post_data(self.tax_password_url,{'corpid':self.corpid,'staff_is_default':0,'tax_staff_pwd':0,'tax_staff_id':staff_data['id'],'msg':'未匹配到报税员'})
                    return False
                # 如果是计算结果错误,可重试
                if '计算结果输入有误' in err_msg:
                    login_times += 1
        ...
        try:
            # 判断报税员验证是否出来
            tax_staff_num = driver.execute_script("return $('#syzSelect option').length")
            print('tax_staff_num',tax_staff_num)
            # 认证人已经加载完毕
            if tax_staff_num > 0:

                # 使用名称从数据中心进行匹配
                print('staff_data',staff_data)
                match_tax_staff = False
                for i in range(tax_staff_num):
                    op_text = driver.execute_script("return $('#syzSelect option:eq(%s)').text()" % i)
                    # print('op_text',op_text)
                    # 还用姓名或身份证后四位匹配
                    if staff_data['uname'] in op_text:
                        print('op_text',op_text)
                        match_tax_staff = True
                        if i != 0:
                            driver.execute_script("$('#syzSelect option:eq(%s)').attr('selected',true);$('#syzSelect option:eq(%s)').change()"%(i,i))
                        #输入密码
                        driver.execute_script("$('#syzYzLx input[name=password]').val('%s')" % staff_data['tax_pwd'])
                        break
                # 没有匹配上报税人 
                if match_tax_staff == False:
                    print('未匹配到报税员')
                    htool.post_data(self.tax_password_url,{'corpid':self.corpid,'staff_is_default':0,'tax_staff_pwd':0,'tax_staff_id':staff_data['id'],'msg':'未匹配到报税员'})
                    return False
                driver.execute_script("syzRealLogin()")
                time.sleep(3)
                # 检验报税员密码弹框
                layer_dialog_num = driver.execute_script("return $('.layui-layer-dialog').length")
                # print('layer_dialog_num',layer_dialog_num)
                if layer_dialog_num == 1:
                    pass_err = driver.find_element_by_class_name("layui-layer-dialog")
                    err_msg = pass_err.find_element_by_xpath("./div[@class='layui-layer-content']").text
                    print('err_msg',err_msg)
                    if ('您的密码输入错误' in err_msg or '密码已输错' in err_msg or '自然人账号不存在' in err_msg) and '查看' not in err_msg:
                        print('报税员密码错误',self.corpid,staff_data['id'])
                        htool.post_data(self.tax_password_url,{'corpid':self.corpid,'tax_staff_pwd':0,'tax_staff_id':staff_data['id'],'msg':'报税密码错误，请及时修改'})
        ...
        try:
            ...
            if 'login' in curr_link:
                ...
                self.driver.execute_script("all_corp=[];$_ajax({url: url_kqxx,success: function(c) {all_corp = c;}});")
                sleep(3)
                all_corp = self.driver.execute_script("return all_corp;")
                if len(all_corp) > 0:
                    for cc in all_corp:
                        if cc['KQCCSZTDJBZ'] == 'N':
                            self.fuser['uuid'] = cc['DJXH']
    ...
    # 登录验证加提交
    def login_verify_submit(self,driver):
        try:
            verify_code = driver.find_element_by_xpath("//form[@id='fm3']/div/input[@name='yzm']")
            parse_code = self.parse_action()
            # print('parse_code',parse_code)
            ...
            print(e)
            ver_handle = HfSwVerify(driver)
            ver_handle.verify_image()
    ...
    def restart_driver(self):
        hlogin = HLogin(self.run_id)
        self.driver = hlogin.restart_driver(self.driver)
    ...
    def get_tax_detail(self,zsxmDm = '',get_ret = False):
        ...
        if sb_data and sb_data.status_code == 200:
            ...
            for d in tax_json['data']:
                ...
                # 匹配模板关键词找到解析模板
                parse_match_temp = {}
                parse_common_data = {"qmldse":qmldse,"szdm":d['YZPZZL_DM']}
                ...
                link = htool.open_cell(d)
                # print('parse_common_data',parse_common_data)
                # 过滤掉无连接的数据
                ...
                if parse_match_temp:
                    ...
                    if sb_data.status_code == 200:
                        ...
                        # print(parse_match_temp)
                        # print(sb_data.text)
                        initData = re.search(r''+parse_match_temp['re_match_name']+'.{10,};',sb_data.text).group(0)
                        initData = initData.replace(parse_match_temp['re_match_name'],'').strip("\";'").strip("JSON.parse('").strip("')")
            ...
            for ss_item in ss_data:
                ...
                fill_date = "%s%s" % (fill_date_arr[0],fill_date_arr[1])
    ...
    # 获取增值税报税相关数据
    def get_zzs_data(self):
        rt_zzs_data = {"corpid":self.corpid,"syn_zzs_data_status":0,"zzs_data":''}
        self.ready_tax_info()
        sb_data = {}
        if self.zzs_sb == True:
            htool = HTool()
            zz_link_arr = self.get_tax_detail('10101',True)
            if len(zz_link_arr) == 0:
                return False
            else:
                rt_zzs_data['fill_date'] = zz_link_arr[0]['fill_date']
                self.driver.get(zz_link_arr[0]['sheet_detail_link'])    
            # print('zz_link',zz_link)
            # time.sleep(5000)
            alert_dic = htool.driver_close_alert(self.driver)
            if len(alert_dic) > 0:
                for it in alert_dic:
                    if '获取已申报信息失败' in it:
                        login_ret = False
            ...
            if '一般纳税人' in self.driver.title:
                sb_data['tax_type'] = 1
                self.main_frame = self.driver.find_element_by_xpath('//div[@id="iframes"]/iframe[@name = "Z00"]')
                self.driver.switch_to.frame(self.main_frame)
                # 获取增值税本期销项金额
                xx_tab_li = ['ybhwjlw_bys_1','ybhwjlw_bys_5','ybhwjlw_bys_7','ybhwjlw_bys_8','jzjthwjlw_bys_1']
                xx_tab_val = []
                for tab_name in xx_tab_li:
                    tab_len = self.driver.execute_script("return $('#sbbxxForm input[name=%s]').length;" % tab_name)
                    if tab_len != 0:
                        tab_val = self.driver.execute_script("return $('#sbbxxForm input[name=%s]').val();" % tab_name)
                        print('tab_val',tab_val)
                        xx_tab_val.append(tab_val)
                sb_data['bqxxje'] = htool.rt_sum_val(xx_tab_val)

                # 获取本期销项税额
                xxse1 = self.driver.execute_script("return $('#sbbxxForm input[name=ybhwjlw_bys_11]').val();")
                sb_data['bqxxse'] = htool.rt_sum_val([xxse1])

                # 获取留底税额
                ldse1 = self.driver.execute_script("return $('#sbbxxForm input[name=ybhwjlw_bys_20]').val();")
                sb_data['ldse'] = htool.rt_sum_val([ldse1])

                self.driver.switch_to.parent_frame()
                # 获取进项金额与税额
                self.driver.execute_script("openFrame(2);")
                time.sleep(2)
                self.iframe = self.driver.find_element_by_xpath('//div[@id="iframes"]/iframe[@name = "Z02"]')
                self.driver.switch_to.frame(self.iframe)
                # 进项金额
                jxje1 = self.driver.execute_script("return $('#bqjxsemxbForm input[name=je_12]').val();")
                sb_data['bqjxje'] = htool.rt_sum_val([jxje1])

                jxse1 = self.driver.execute_script("return $('#bqjxsemxbForm input[name=se_12]').val();")
                sb_data['bqjxse'] = htool.rt_sum_val([jxse1])
                sb_data['syn_zzs_data_status'] = 1
            elif '小规模' in self.driver.title:
                sb_data['tax_type'] = 0
                self.main_frame = self.driver.find_element_by_xpath('//div[@id="iframes"]/iframe[@name = "zzsxgmsb"]')
                self.driver.switch_to.frame(self.main_frame)
                # 获取增值税本期销项金额
                xx_tab_li = ['yzzzsbhsxse_1_1','yzzzsbhsxse_1_2','xsczbdcbhsxse_4_1','xsczbdcbhsxse_4_2','xssygdysgdzcbhsxse_7_1','xssygdysgdzcbhsxse_7_2','msxse_9_1','msxse_9_2','ckmsxse_13_1','ckmsxse_13_2']
                xx_tab_val = []
                for tab_name in xx_tab_li:
                    tab_len = self.driver.execute_script("return $('#sbbxxForm input[name=%s]').length;" % tab_name)
                    if tab_len != 0:
                ...
                # 获取本期应纳税额
                ynse_hw = self.driver.execute_script("return $('#sbbxxForm input[name=ynsehj_22_1]').val();")
                ynse_fw = self.driver.execute_script("return $('#sbbxxForm input[name=ynsehj_22_2]').val();")
                ynse = htool.rt_sum_val([ynse_hw,ynse_fw])
                # 获取本期应补退
                ybt_hw = self.driver.execute_script("return $('#sbbxxForm input[name=bqybtse_24_1]').val();")
                ybt_fw = self.driver.execute_script("return $('#sbbxxForm input[name=bqybtse_24_2]').val();")
                ybt = htool.rt_sum_val([ybt_hw,ybt_fw])
                sb_data['bqxxse'] = round(ynse - ybt,2)
                sb_data['syn_zzs_data_status'] = 1
            # if post_ret.status_code == 200:
            #     print('上传结果',post_data,post_ret.text)
            # else:
            #     print('上传失败',post_data)
            # self.driver.quit()
            # time.sleep(3)
        else:
            print('本月无需申报增值税')
        rt_zzs_data['zzs_data'] = json.dumps(sb_data)
        print('rt_zzs_data',rt_zzs_data)    
    ...
    def parseTax(self,data,template,parse_common_data):
        try:
            # print('待解析json数据',data)
            json_data = json.loads(data)
        except:
            print("解析json数据错误 %s" % data)
            return[]
        ...
        if template['temp_id'] == '2':
            if 'sb_cxs_tysbxx' in json_data:
                match_rows = json_data['sb_cxs_tysbxx']
            elif 'yyssbbdxxVO' in json_data:
        ...
        if template['temp_id'] == '4':
            if 'fjsSbbdxxVO' in json_data:
                ...
                match_rows = json_data['zzssyyxgmnsrySbSbbdxxVO']['zzsxgmflzl']['flzlForm']
            elif 'taxML' in json_data:
                match_rows = json_data['taxML']['zzsybsbSbbdxxVO']['zzssyyybnsr_zb']['zbGrid']['zbGridlbVO']
            else:
                print(data)
                self.insert_log("未找到解析附加税数据的方法")
        ...
        for match_row in match_rows:
            ...
            for f in template['match']:
                try:
                    ...
                    pass
            if 'zspm_dm' in match_row and 'levy_items' in parse_data and parse_data['levy_items'] == '':
                parse_data['levy_items'] = self.pmdm_dic[parse_data['zspm_dm']]
    ...
    # 获取社保申报详情
    def get_sb_detail(self,all_month = True,check_sb_kk = False):
        sb_list = []
        htool = HTool()
        self.has_sb = True
        ...
        for sb_period in sb_list:
            ...
            fill_date_arr = sb_list[sb_period]['fill_date'].split('-')
            fill_date = "%s%s" % (fill_date_arr[0],fill_date_arr[1])
            
            post_sb_field = {"corpid":self.corpid,'period':sb_list[sb_period]['period'],'tax_type':1,'fill_date':fill_date,"msg":"自动更新[所属期：%s - 已申报社保数据]完成" % sb_list[sb_period]['period'],"data":json.dumps([sb_list[sb_period]])}
            self.post_sb_field = post_sb_field
            res = htool.post_data(self.tax_update_url,post_sb_field)
            if res.status_code == 200:
                try:
                    res_text = json.loads(res.text)
                    if res_text['code'] != 0:
                        print(self.corpname+":获取社保信息失败 ",res.text)
                    else:
                        print('上传社保',fill_date,res.text)
                        return True
                except: 
                    pass
        return False
    ...
    def sb_gl(self,sb_list):
        ...
        for sb in sb_list:
            ...
            if fill_date in gl_dict:
                ...
                gl_dict[fill_date] = {"categories_name":"社保","szdm":"BDA0610222","voucher_number":sb['voucher_number'],"period":period,"start_date":sb['start_date'],"stop_date":sb['stop_date'],'fill_date':sb['fill_date'],"bqybtse":float(sb['bqybtse']),"bqynse":float(sb['bqynse']),"json_detail":[json.dumps(sb)]}
    ...
    def do_tax_kk_action(self,bank_num):
        ...
        if kk_item_num == 0:
            # 检测是否有异常扣款
            search_data = {'uuid':self.fuser['uuid']}
            try:
                get_err_data = htool.post_data(self.kk_err_search_url,search_data,self.driver)
                err_data = json.loads(get_err_data.text)
                if 'data' in err_data and len(err_data['data']['ztbmJkxx']) > 0:
                    msg = '存在状态不明税款信息'
                    kk_post_data['kk_status'] = 2
                    kk_post_data['msg'] = msg
                    return kk_post_data
            except Exception as e:
                print(e)
                return kk_post_data
    ...
    def xnw_dkfp_search(self):
        ...
        if '绑定手机号' in err_bind_mess:
            ...
            self.driver.switch_to.frame(self.iframe)
            time.sleep(2)
            corp_num = self.driver.execute_script("return $('table.qytable #tableBody tr').length")
            if corp_num == 1:
                self.driver.execute_script("lineClickFunc(0)")
            else:
                for j in range(corp_num):
                    line_corp = self.driver.execute_script("return $('table.qytable #tableBody tr:eq(%d) td:eq(2)').text()" % j)
                    # print('line_corp',line_corp)
                    if self.corpname == line_corp:
                        self.driver.execute_script("lineClickFunc(%d)" % j)
                        break
            time.sleep(2)
            try:
        ...
        search_link = self.xnw_search_list_url + "pageSize=9999&sqBegintime=%s&sqEndtime=%s" % (st_time,end_time)
        ...
        print("信诺网代开发票上传",len(agent_list))
    ...
    def dk_check(self):
        ...
        if dkfp_ret.status_code == 200:
            ...
            # 无代开发票
            if len(dkfp_data['data']) == 0:
                return True
            filter_arr = []
            # 判断信诺网与税务局代开情况
            dk_checked_data = {"sw_dk":0,"xnw_dk":0,"corpid":self.corpid,"kjrqq":kjrqq,"kjrqz":kjrqz}
            for i in range(len(dkfp_data['data'])):
                if dkfp_data['data'][i]['fpzlDm'] in ['1160','1130'] or '专用发票' in dkfp_data['data'][i]['fpzlMc']:
                    dk_checked_data['sw_dk'] = 1
                else:
                    dk_checked_data['xnw_dk'] = 1
            # 根据当前获取的匹配结果与后台对比
            print('代开发票情况',dk_checked_data)
            dk_match = htool.post_data(self.checked_dkfp_url,dk_checked_data)
            if dk_match.status_code == 200:
                print('dk_match.text',dk_match.text)
                match_ret = json.loads(dk_match.text)
                return bool(match_ret['checked'])
            return False
    ...
    def sw_dkfp_search(self):
        ...
        if dkfp_ret.status_code == 200:
            ...
            # print('简要信息',json.dumps(dkfp_data['data']))
        ...
        if dkfp_data.status_code == 200:
            ...
            if float(dkfp_json['data']['total']) > 0 and len(dkfp_json['data']['rows']) > 0:
                ...
                for dk in dkfp_json['data']['rows']:
                    ...
                    for input_li in input_obj:
                        k_name = input_li.get('name')
                        if k_name != None:
                            if k_name not in ['hwlwmc','ggxh','hlsl','dj','je','jsxj','kce','slzsl','se','jylb']:
                                row_detail[k_name] = input_li.get("value")
    ...
    def ready_tax_info(self):
        ...
        if sb_data and sb_data.status_code == 200:
            try:
                ...
                yhs_ac_sb_status = False
                if len(tax_data) > 0:
                    for tax_it in tax_data:
                        if '一般纳税人' in tax_it['YZPZZL_MC']:
                            self.is_ybnsr = True
                        post_bddm = ''
                        if tax_it['BDDM'] == 'YHS0794' and tax_it['SKSSQQ'] == tax_it['SKSSQZ']:
                            yhs_ac_sb_status = True

                        if 'BDDM' in tax_it:
                            post_bddm = tax_it['BDDM']
                        elif 'YZPZZL_DM' in tax_it:
                            post_bddm = tax_it['YZPZZL_DM']    
                        already_tax.append(post_bddm+':'+tax_it['SKSSQQ'])
            except:
                print('解析已申报数据出错')
                return False
        ...
        if tax_info:
            ...
            for it in rt_dict:
                ...
                if it[4] == '次':
                    ss_period = 0
                if it[2] == '印花税':
                    if ss_period in self.yhs_item_list:
                        self.yhs_item_list[ss_period].append(it[3])
                    else:
                        self.yhs_item_list[ss_period] = [it[3]]
                    if ss_period > 0:
                ...
                mm = time.strftime("%m")
                if (now_month == it[6] and it[3] != '经营所得') or it[1] != '自行申报':
                    continue
                ...
                zzs_dict = {}
                for cf in tax_bd:
                    # print(it[2],it[3],cf['keyword'])
                    if ((it[2] in cf['keyword'] or it[0] in cf['keyword']) or (it[2] in cf['keyword'] and it[3] in cf['keyword'])) and it[4] in cf['keyword']:
                        sz_bddm = cf['bddm']
                        pmdm = ''
                        if 'zspmDm' in cf:
                            pmdm = cf['zspmDm']
                        if (sz_bddm == 'YHS0794' and ss_period == 0  and yhs_ac_sb_status == True):
                            # print('印花税已按次申报')
                            continue
                        if (sz_bddm not in self.sb_ignore_list and (sz_bddm + pmdm + it[4]) not in tax_dict) and ((sz_bddm + pmdm + it[4]) not in zzs_dict) and (sz_bddm+pmdm+":"+ssqq not in already_tax):
        ...
        print('self.yhs_item_list',self.yhs_item_list)
    ...
    # 获取未上传社保缴纳信息企业
    def get_sb_jk_data(self,kk_date):
        htool = HTool()
        post_res = htool.post_data(self.ready_sb_jk_detail_url,{"kk_data":kk_date})
        if post_res.status_code == 200:
            return json.loads(post_res.text)
        else:
            return {}
    ...
    def social_insurance(self,sw_sb_ret):
        ...
        if len(sb_jn_ret) > 0:
            ...
            if post_data['comp_status'] == 1 or post_data['comp_status'] == 9:    
    ...
    def check_sb_result(self,result):
        ...
        if result['comp_status'] == 1:
            ...
            sleep(3)
            # 获取本月实际应缴纳社保款项
            sb_ret = self.get_sb_detail(False,True)
            # print('最终上传社保结果',sb_ret)
            if sb_ret == True:
                if self.has_sb == True:
                    # print('post_sb_field',self.post_sb_field)
    ...
    def tax_si_upload(self,page=0,page_size = 20000,tax_data = []):
        self.sbrqq = time.strftime("%Y%m",time.localtime())
        self.sbrqz = time.strftime("%Y%m",time.localtime())
        # self.sbrqq = '202006'
        # self.sbrqz = '202006'
        print('社保数据获取',self.sbrqq,self.sbrqz,page)
        ...
        for gl in gl_data:
            ...
            for sig in rows:
                ...
                if user_sign not in users:
                    kk_date = sig['aae002']
                    rad_int = random.randint(1000,9000)
                    data_text = "%s%s%s%s" %(self.corpid,self.sbrqz,user_sign,rad_int)
                    d_id = hashlib.md5(data_text.encode(encoding='UTF-8')).hexdigest()
                    users[user_sign] = {"data_id":d_id,"kk_date":sig['aae002'],"identify":sig['aac002'],"type":sig['aaa115'],"type_name":sig['aaa115_mc'],"period":sig['aae003'],"uname":sig['aac003'],"total_money":float(sig['aae022'])+float(sig['aae020']),"gr":float(sig['aae022']),"dw":float(sig['aae020'])}
                else:
    ...
    def tax_sb(self):
        ...
        if self.is_ybnsr == False:
            dk_check = self.dk_check()
            if dk_check == False:
                msg = '检测到代开未勾选，等待上传后重试'
                print(msg)
                return msg
        # print('报税核定结果',hd_ret)
        htool = HTool()
    ...
    def do_tax_sb(self,tax_info):
        ...
        if bddm == 'TYSB100':
            ...
            if len(mes_ty) > 0:
                for ms_it in mes_ty:
                    if '完成汇总报送' in ms_it or '上期未申报' in ms_it or '该纳税人重复申报' in ms_it or '无法办理当前业务' in ms_it:
                        return ms_it.replace('【页面即将关闭】','') + '[通用申报失败]'
            ...
            for sb_item in parse_sb_data:
                ...
                if 'zspmDm' in sb_item and sb_item['zspmDm'] == '302210400':
                    ...
                    if 'pmmc' not in sb_data or 'shouru' not in sb_data:
                        return '水利基金获取报税数据失败'
                    print('申报数据',sb_data)
        ...
        # 企业所得税(暂未区分A类或B类)
            ...
            for ms_it in msg_dic:
                if '上期未申报' in ms_it or '该纳税人重复申报' in ms_it:
                    return ms_it.replace('【页面即将关闭】','') + "[所得税申报失败]"
                elif '无需进行申报' in ms_it:
                    self.sb_ignore_list.append(tax_info['bddm'])
                    return ms_it.replace('【页面即将关闭】','') + "[所得税无需申报]"

            last_per = 0        
            mm = time.strftime("%m")        
            # # 获取上季度收入 fhtjxwqyyhjzsdseLj
            if int(mm) > 6:
                st_month,ed_month = htool.month_get(3)
                search_sr_cond = {'sbrqq':st_month,'sbrqz':ed_month,'ssqq':'','ssqz':'','zsxmDm':'10104','uuid':self.fuser['uuid']}
                sb_data = htool.post_data(self.tax_data_url,search_sr_cond,self.driver)
                if sb_data and sb_data.status_code == 200:
                    try:
                        # print('统计数据结果',sb_data.text)
                        tax_json = json.loads(sb_data.text)
                    ...
                    if 'data' in tax_json and len(tax_json['data']) > 0:
                        link = htool.open_cell(tax_json['data'][0])
                        # print('上季度企业所得税链接',link)
                        sb_data = htool.get_data(link,self.driver)
                        # print(sb_data.text)
                        yysr_match = re.search(r'"fhtjxwqyyhjzsdseLj":"(\d{0,}\.?\d{0,})"',sb_data.text)
                        print('yysr_match',yysr_match)
                        if yysr_match:
                            last_per = yysr_match.group(0).replace('"fhtjxwqyyhjzsdseLj":"','').replace('"','')

                        print('上季度利润',last_per)    
            ...
            self.main_frame = self.driver.find_element_by_xpath('//div[@id="iframes"]/iframe[@name = "%s"]' % main_name)
            self.driver.switch_to.frame(self.main_frame)
            # 实际已缴纳所得税额
            ...
            if float(yysr) > 0:
                ...
                #1 营业收入
                self.driver.execute_script("$('#sbbxxForm input[name=yysrLj]').val(%s).trigger('change');" % round(yysr,2))
                # 客户交税，填写客户交税部分利润，不交税控制利润-2000~-5000
                if float(sb_data['estimate_qysds_money']) > 0:
                    # 客户交税
                    lr_total = (float(sb_data['estimate_qysds_money'])+float(last_per))/0.05
                else:
                    # 客户不交税
                    lr_total = float(last_per)
            ...
            now_j = lart_j + 1
            if jc_rs == '' or jm_rs == '' or jc_ze == '' or jm_ze == '':
                jc_rs = jm_rs = jc_ze = jm_ze = '1'
        ...
        # 印花税
            ...
            # 打开印花税税源页面
            self.driver.get(self.yhs_base_page_url+self.fuser['uuid'])
            aj_dict = ['01','04','07','10']
            # time.sleep(8000)
            # now_month = time.strftime("%Y-%m-01")
            mm = time.strftime("%m")
            ...
            if mm in aj_dict:
                ...
                month_range = 1
            time_start,time_end = htool.month_get(month_range)
            # 如果本月不是季报，并且只有按次申报，采集时间改成一天
            if ss_period == 0:
                sscj_time = time_end
            else:
                sscj_time = time_start
            self.driver.execute_script("$('#skssqq').val('%s');$('#skssqz').val('%s');queryYhssyxx();" % (sscj_time,time_end))


            # 税源采集数(暂未区分印花税或其它税种，请注意)
            # $('#yhssyTable tbody tr').length
            time.sleep(1)
            sr_len = self.driver.execute_script("return $('#yhssyTable tbody tr').length;")
            sy_cj = self.driver.execute_script("return $('#yhssyTable tbody tr:eq(0) td').length;")
            sr_exist = False
            if sy_cj>2:
                for sr_index in range(sr_len):
                    sk_time = self.driver.execute_script("return $('#yhssyTable tbody tr:eq(%d) td:eq(1)').text();" % sr_index)
                    print('sk_time',sk_time)
                    if sk_time == sscj_time:
                        sr_exist = True
            print('sr_exist',sr_exist)
            # time.sleep(8000)
            if sr_exist == False:
                # 添加税源采集
                print('添加税源采集')
                self.driver.execute_script('openDialog("addYhssy");$("input#skssqq1").val("%s");$("input#skssqz1").val("%s").change();' % (sscj_time,time_end))
                it_start = 0
                ...
                sr_total = sb_data['shouru']
                if sb_data['pmmc'] != '':
                    sel_opt = sb_data['pmmc']
                elif self.yhs_item_list[ss_period][0] != '资金账簿':
                    sel_opt = self.yhs_item_list[ss_period][0]
                elif len(self.yhs_item_list[ss_period]) > 1:
                    sel_opt = self.yhs_item_list[ss_period][1]
                ...
                if ss_period == 0:
                    it_start = 0
                    # 检查按次是否需要重新选择
                    skss_q = skss_z = time_end
                    for ac_it in self.yhs_item_list[ss_period]:
                        self.driver.execute_script("$('#acsbList .sbt-table-option-warp .sbt-table-add').click()")
                        time.sleep(1)
                        self.driver.execute_script("$(\"#acsbList .sbt tbody tr:eq(%d) .sb_table_select select\").find(\"option:contains('%s')\").attr(\"selected\",true);" % (it_start,ac_it))
                        self.driver.execute_script("$(\"#acsbList .sbt tbody tr:eq(%d) .sb_table_select select\").change();" % it_start)
                        self.driver.execute_script("$('#acsbList .sbt tbody tr:eq(%d) input[name=skssqq]').val('%s').change();" % (it_start,skss_q))
                        self.driver.execute_script("$('#acsbList .sbt tbody tr:eq(%d) input[name=skssqz]').val('%s').change();" % (it_start,skss_z))
                        it_start += 1
                    ...
                    if float(sr_total) > 0:
                        aq_start = 0
                        match_yh = False
                        for i in range(len(self.yhs_item_list[ss_period])):
                            first_sr_text = self.driver.execute_script("return $('#aqsbList tbody select[name=zspmDm]:eq(%d)').find(':selected').text()" % aq_start)
                            # # 购销合同不为按次
                            # if first_sr_text != '资金账簿':
                            #     it_start = 0
                            # elif len(self.yhs_item_list[ss_period]) > 1:
                            #     it_start += 1
                            # else:
                            #     return '唯一选项，资金账簿不可填写收入，印花税申报失败'
                            if (pmmc == '' and first_sr_text != '资金账簿') or (pmmc != '' and first_sr_text == pmmc):
                                match_yh = True
                                break
                            aq_start += 1    
                        if match_yh == True:
                            self.driver.execute_script("$('#aqsbList .sbt tbody tr:eq(%d) input[name=jsjehjs]').val(%s).trigger('change');" % (aq_start,sr_total))
                        else:
                            return '按期申报未能匹配税目，印花税申报失败，请手动申报'
                time.sleep(2)
                self.driver.execute_script("saveData();")
                time.sleep(2)
            self.driver.get(self.other_tax_dic_url+self.fuser['uuid'])
            time.sleep(2)
            self.iframe = self.driver.find_element_by_xpath('//div[@id="iframes"]/iframe[@name = "jbxxb"]')
            self.driver.switch_to.frame(self.iframe)
            self.driver.execute_script("$('.sbt-box input.sbt-checkbox-all').click();")
            self.driver.switch_to.default_content()
            
            self.driver.execute_script("showNext();")
            time.sleep(3)
            self.driver.execute_script("shenbao();")
            time.sleep(3)
            search_ret_num = self.driver.execute_script("return $('#sb_open .sb_open_title_text').length;")
            if search_ret_num > 0:
                self.driver.execute_script("ok();")
            else:
                print('综合申报结果未知')
                return ''
        ...
        # 一般纳税人增值税 
            ...
            # print('增值税链接',tax_url)
            self.driver.get(tax_url)
            msg_dic = htool.driver_close_alert(self.driver,5)
            print(msg_dic)
            # 税控盘未抄税情况
            for ms_it in msg_dic:
                if '完成汇总报送' in ms_it or '上期未申报' in ms_it or '该纳税人重复申报' in ms_it or '无法办理当前业务' in ms_it:
            ...
            print('sb_data',sb_data)
            if len(sb_data) == 0:
                return '可能含有免税项目，请手动检查并根据发票内容申报'
            # 测试数据
            ...
            if float(sb_data['red_rush_tax']) > 0:
                self.driver.execute_script("$('#bqjxsemxbForm input[name=se_20]').val(%s).trigger('change');" % sb_data['red_rush_tax'])
            ...
            print('附表4打开')
            self.driver.switch_to.default_content()
            self.driver.execute_script("openFrame(4);")
            time.sleep(2)
            htool.driver_close_alert(self.driver,3)
            # 检查有没有加计抵减
            self.iframe = self.driver.find_element_by_xpath('//div[@id="iframes"]/iframe[@name = "Z04"]')
            self.driver.switch_to.frame(self.iframe)
            qcye = self.driver.execute_script("return $('#jjdjqkbForm input[name=qcye]').val();")
            if qcye != '0.00':
                self.driver.execute_script("$('#jjdjqkbForm input[name=bqfse]:eq(0)').val(%s);" % round(sb_data['jx_se'] *0.1,2))
            ...
            # 外地预缴
            wdyj_qc = self.driver.execute_script("return $('#sedjqkbForm input[name=qcye_jzfwyzjnsk_1]').val();")
            if float(wdyj_qc.replace(',','')) != 0:
                return '检测到外地预缴有期初余额，请联系财税核实是否有本期发生额并手动填写本期发生额'
            ...
            # 打开增值税减免申报明细表
            self.driver.execute_script("openFrame(5);")
            htool.driver_close_alert(self.driver,3)

            # 填写附表1
            print('附表1填写')
            self.driver.execute_script("openFrame(1)")
            time.sleep(2)
            htool.driver_close_alert(self.driver,3)
            ...
            # 主表填写
            self.driver.execute_script("openFrame(0)")
            print('填写增值税主表')
            ...
            if yn_hj > 0 and sb_data['skp_or_fwf_total'] > 0:
                # 实际优惠金额
                if float(sb_data['skp_or_fwf_total']) > yn_hj:
                    rel_disc = yn_hj
                else:
                    rel_disc = sb_data['skp_or_fwf_total']      
                # 填写附表4
                print('有税控盘优惠，填写优惠')
                self.driver.execute_script("openFrame(4);")
                htool.driver_close_alert(self.driver,3)
                ...
                self.driver.execute_script("$('#sedjqkbForm input[name=bqsjdjse_zzsskxtfy_4]').val(%s).trigger('change');" % rel_disc)
                ...
                if float(sb_data['skp_or_fwf_total']) > 0:
                    ...
                    sleep(2)
                    self.driver.execute_script("$('.sbt select[name=hmc]').find(\"option:contains('0001129914|SXA031900185')\").attr(\"selected\",true);$('.sbt select[name=hmc]').trigger('change');")

                    self.driver.execute_script("$('.sbt input[name=bqfse]').val(%s).trigger('change');" % sb_data['skp_or_fwf_total'])
                    self.driver.execute_script("$('.sbt input[name=bqsjdjse]').val(%s).trigger('change');" % rel_disc)
            ...
            self.driver.switch_to.frame(self.iframe)
            self.driver.execute_script("$('#ysfwkcxmmxForm input[name=bqysfwjghjemsxse_1_6]').val(%s).trigger('change');" % round(ys_fw_kc,2))
        ...
        # 小规模增值税
            ...
            if (total_money/ss_period) > 100000:
                ...
                self.driver.execute_script("$('#sbbxxForm input[name=bqynsejze_18_1]').val(%s).trigger('change');" % round((lw_z_total + lw_p_total)*0.02,2))
                self.driver.execute_script("$('#sbbxxForm input[name=bqynsejze_18_2]').val(%s).trigger('change');" % round((fw_z_total + fw_p_total)*0.02,2))
                if jm_item > 0:
                    # 本期应纳税额减征额
                    jme = math.ceil(jm_item*3)/100

                    self.driver.execute_script("$('#sbbxxForm input[name=bqmse_19_2]').val(%f).trigger('change');" % jme)
                    self.driver.execute_script("$('#sbbxxForm input[name=xwqymse_20_2]').val(%f).trigger('change');" % jme)
            else:
                # 货物及劳务
                ...
                jme = math.ceil((fw_z_total + fw_p_total)*3)/100
                self.driver.execute_script("$('#sbbxxForm input[name=bqmse_19_2]').val(%f).trigger('change');" % jme)
                self.driver.execute_script("$('#sbbxxForm input[name=xwqymse_20_2]').val(%f).trigger('change');" % jme)
            ...
            return "暂不支持消费税及消费附加税申报，请手动申报后重试。[消费税申报失败]"
        elif bddm == 'DQDE642':
            return "暂不支持定期定额税申报，请手动申报后重试。[定期定额税申报失败]"
        else:
            print(bddm,'对应的报税方法正在完善[申报失败]')
        sleep(5)
        fjs_ret = htool.driver_close_alert(self.driver,6)    
